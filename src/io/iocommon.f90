MODULE iocommon
  ...
  INTEGER, PARAMETER :: TYPE_SCRIBBLE = -1, &
      TYPE_ADDITIONAL = 0, &
      TYPE_MESH = 1, &
  ...
  INTEGER, PARAMETER :: PARTICLE_CARTESIAN = 0, &
      PARTICLE_POLAR = 1, &
      PARTICLE_CYLINDRICAL = 2
  INTEGER, PARAMETER :: VAR_CARTESIAN = 0, VAR_PARTICLE = 1

  ! Dimension_Irrelevant is used where the dimensionality isn't needed,
  ! as with particle variables still keep dimensionality as a common
  ! quantity because other than this, they really are very alike
  INTEGER, PARAMETER :: DIMENSION_IRRELEVANT = 0, &
      DIMENSION_1D = 1, &
      DIMENSION_2D = 2, &
      DIMENSION_3D = 3
  ...
  INTEGER :: max_string_len = 40, default_rank = 0
