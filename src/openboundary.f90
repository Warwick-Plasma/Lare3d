  ...
  SUBROUTINE damp_boundaries
    ...
    IF (damping) THEN  
       ...
       DO ix = -2, nx+2
          Do iy = -2, ny+2
             DO iz = -2, nz+2
                r_a = SQRT((xb(ix)**2) + (yb(iy)**2) + (0.25_num * ((zb(iz)+20.0_num)**2))) 
                alpha = 0.5_num*(TANH(-(r_a-90.0_num)/20.0_num)+1.0_num)
                vx(ix,iy,iz) = alpha * vx(ix,iy,iz)
                vy(ix,iy,iz) = alpha * vy(ix,iy,iz)
                vz(ix,iy,iz) = alpha * vz(ix,iy,iz)
             END DO
          END DO
       END DO
          ...
          DO iz = -1, nz+1
             DO iy = -1, ny+1
                ...
                ELSE
                   CALL open_bcs_3
                END IF
                v0 = SQRT(vx(nx+1,iy,iz)**2+vy(nx+1,iy,iz)**2+vz(nx+1,iy,iz)**2)
          ...
          DO iz = -1, nz+1
             DO ix = -1, nx+1
                ...
                ELSE
                   CALL open_bcs_3
                END IF
                v0 = SQRT(vx(ix,ny+1,iz)**2+vy(ix,ny+1,iz)**2+vz(ix,ny+1,iz)**2)
                v1 = SQRT(vxbc(0)**2+vybc(0)**2+vzbc(0)**2)
          ...
          DO iy = -1, ny+1
             DO ix = -1, nx+1
                ...
                ELSE
                   CALL open_bcs_3
                END IF
                v0 = SQRT(vx(ix,iy,nz+1)**2+vy(ix,iy,nz+1)**2+vz(ix,iy,nz+1)**2)
                v1 = SQRT(vxbc(0)**2+vybc(0)**2+vzbc(0)**2)
...
! open bc when bx=0 
    ...
    pg = 0.5_num*(pstar(1)+pstar(2)+rhofar*cffar*(vstar(1)-vstar(2)))
    ...
    uxg = 0.5_num*(vstar(1)+vstar(2)+(pstar(1)-pstar(2))/(rhofar*cffar))
    vxbc(0) = uxg
    vybc(0) = vybc(1)
    vzbc(0) = vzbc(1)
...
! open bc when bperp=0 
    ...
    rbc(0) = MAX(rhog,none_zero)
    ebc(0) = MAX(pg-pmagg,none_zero)/(gamma-1.0_num)/rbc(0)
    uxg = 0.5_num*(uxstar(1)+uxstar(2)+(pstar(1)-pstar(2))/(rhofar*c0far))
    vxbc(0) = uxg
  ...
  SUBROUTINE open_bcs_3
    ...
    cx = SQRT(bxbc(1)**2/rbc(1))
    ct = SQRT((bybc(1)**2+bzbc(1)**2)/rbc(1))
    cf = SQRT(0.5_num*((c0**2+cx**2+ct**2)+SQRT((c0**2+cx**2+ct**2)**2&
         -4.0_num*c0**2*cx**2)))
    cs = SQRT(0.5_num*((c0**2+cx**2+ct**2)-SQRT((c0**2+cx**2+ct**2)**2&
         -4.0_num*c0**2*cx**2)))
    ...
    g = (lambdastar(4)-lambdastar(3))/(rhofar*csfar) + (uystar(3)+uystar(4))
