...
MODULE yremap
  ...
  REAL(num), DIMENSION(:,:,:), ALLOCATABLE :: rho1, dm, cv2, flux, dyb1, rho_v, rho_v1
...
CONTAINS
  ...
  SUBROUTINE remap_y
    ...
    DO iz = -1, nz+2
       DO iy = -1, ny+2
          DO ix = -1, nx+2
             ...
             dyb1(ix,iy,iz) = dyb(iy) + (vyb - vybm) * dt !dyb before remap
          END DO
       END DO
    END DO
    ...
    DO iz = 1, nz
       DO iy = 1, ny
          DO ix = 1, nx
             iym = iy - 1
             rho(ix,iy,iz) = (rho1(ix,iy,iz)*cv1(ix,iy,iz) + dm(ix,iym,iz)-dm(ix,iy,iz)) &
                  /cv2(ix,iy,iz)
          END DO
       END DO
    END DO
       ...
       DO iy = 1, ny
          DO ix = 1, nx
             ...
             energy(ix,iy,iz) = (energy(ix,iy,iz)*cv1(ix,iy,iz)*rho1(ix,iy,iz) &
                  +flux(ix,iym,iz)-flux(ix,iy,iz))/(cv2(ix,iy,iz)*rho(ix,iy,iz))
          END DO
       END DO
    END DO
    ...
    DO iz = 0, nz
       DO iy = -1, ny+1
          DO ix = 0, nx    ! vertex density before remap
             ixp = ix + 1
             iyp = iy + 1
             izp = iz + 1
    ...
    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = 0, nx
             iym = iy - 1
             rho_v1(ix,iy,iz) = (rho_v(ix,iy,iz)*cv1(ix,iy,iz)+dm(ix,iym,iz)-dm(ix,iy,iz))&
                  /cv2(ix,iy,iz)
          END DO
       END DO
    END DO
       ...
       DO iy = 0, ny
          DO ix = 0, nx
             ...
             vz(ix,iy,iz) = (rho_v(ix,iy,iz)*vz(ix,iy,iz)*cv1(ix,iy,iz)  &
                  +flux(ix,iym,iz)-flux(ix,iy,iz))/(cv2(ix,iy,iz)*rho_v1(ix,iy,iz))
          END DO
       END DO
    END DO
       ...
       DO iy = 0, ny 
          ...
          DO ix = 0, nx
             ...
             w4 = (2.0_num-w5)*ABS(w1)/dyc(iy)&
                  +(1.0_num+w5)*ABS(w2*vad_p + w3 * vad_m)/(dyc(iym)*vad_p+dyc(iyp)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
             w6 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dyb(iy)*vad_p+dyb(iyp)*vad_m), ABS(w1), ABS(w2*vad_p + w3*vad_m))
    ...
    DO iz = 0, nz+1 
       DO iy = 0, ny 
          ...
          DO ix = 0, nx+1
             ...
             v_advect = (vy1(ix,iy,iz)+vy1(ixm,iy,iz)+vy1(ix,iy,izm)+vy1(ixm,iy,izm)) &
                  / 4.0_num
             ...
             w5 = ABS(v_advect) * dt / (dyb1(ix,iy,iz)*vad_p + dyb1(ix,iyp,iz)*vad_m)
             w4 = (2.0_num-w5)*ABS(w1)/dyc(iy)+(1.0_num+w5)*ABS(w2*vad_p+w3*vad_m)/(dyc(iym)*vad_p+dyc(iyp)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p + w3*vad_m))
             w6 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dyb(iy)*vad_p+dyb(iyp)*vad_m), ABS(w1), ABS(w2*vad_p + w3 * vad_m))
    ...
    DO iz = 0, nz 
       DO iy = 0, ny 
          ...
          DO ix = 0, nx
             ...
             w6 = ABS(dm(ix,iy,iz)) &
                  / ((rho1(ix,iy,iz)*dyb1(ix,iy,iz)*vad_p + rho1(ix,iyp,iz)*dyb1(ix,iyp,iz)*vad_m)*dxb(ix)*dzb(iz))
             w4 = (2.0_num-w5)*ABS(w1)/dyc(iy)+(1.0_num+w5)*ABS(w2*vad_p + w3 *vad_m)/(dyc(iym)*vad_p+dyc(iyp)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
             w5 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dyb(iy)*vad_p+dyb(iyp)*vad_m), ABS(w1), ABS(w2*vad_p + w3 *vad_m))
    ...
    DO iz = 0, nz 
       DO iy = -1, ny 
          ...
          DO ix = 0, nx
             ...
             w6 = ABS(dm(ix,iy,iz)) &
                  / ((rho_v(ix,iy,iz)*dyb1(ix,iy,iz)*vad_p + rho_v(ix,iyp,iz)*dyb1(ix,iyp,iz)*vad_m)*dxc(ix)*dzc(iz))
             w4 = (2.0_num-w5)*ABS(w1)/dyb(iyp)&
                  +(1.0_num+w5)*ABS(w2*vad_p + w3 * vad_m)/(dyb(iy)*vad_p + dyb(iyp2)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p + w3 * vad_m))
             w5 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dyc(iy)*vad_p + dyc(iyp)*vad_m), ABS(w1), ABS(w2*vad_p + w3 * vad_m))
       ...
       DO iz = 0, nz 
          DO iy = 0, ny-1 
             ...
             DO ix = 0, nx
                ...
                dk =   (vx(ix,iyp,iz) - vx(ix,iy,iz))   &
                     * (flux(ix,iy,iz) - 0.5_num*(vx(ix,iyp,iz)+vx(ix,iy,iz)))  &
       ...
       DO iz = 0, nz 
          DO iy = 0, ny-1 
             ...
             DO ix = 0, nx
                ...
                dk =   (vz(ix,iyp,iz) - vz(ix,iy,iz))   &
                     * (flux(ix,iy,iz) - 0.5_num*(vz(ix,iyp,iz)+vz(ix,iy,iz)))  &
       ...
       DO iz = 0, nz 
          DO iy = 0, ny-1 
             ...
             DO ix = 0, nx
                ...
                dk = (vy(ix,iyp,iz) - vy(ix,iy,iz))   &
                     * (flux(ix,iy,iz) - 0.5_num*(vy(ix,iyp,iz)+vy(ix,iy,iz)))  &
  ...
  SUBROUTINE dm_y_bcs
    ...
    CALL MPI_SENDRECV(dm(0:nx+1,1,0:nz+1), (nx+2)*(nz+2), mpireal, down, tag,  &
         dm(0:nx+1,ny+1,0:nz+1), (nx+2)*(nz+2), mpireal, up, tag, comm, status, errcode)
    IF (up == MPI_PROC_NULL) dm(0:nx+1,ny+1,0:nz+1) = dm(0:nx+1,ny,0:nz+1)

    CALL MPI_SENDRECV(dm(0:nx+1,ny-1,0:nz+1), (nx+2)*(nz+2), mpireal, up, tag,  &
         dm(0:nx+1,-1,0:nz+1), (nx+2)*(nz+2), mpireal, down, tag, comm, status, errcode)
    IF (down == MPI_PROC_NULL) dm(0:nx+1,-1,0:nz+1) = dm(0:nx+1,0,0:nz+1)
