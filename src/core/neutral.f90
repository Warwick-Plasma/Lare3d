...
MODULE neutral
  ...
  IMPLICIT NONE
  PRIVATE
  PUBLIC :: perpendicular_resistivity, newton_relax, neutral_fraction, setup_neutral, get_neutral
...
CONTAINS
  ...
  SUBROUTINE setup_neutral
    ...
    eta_bar = (0.5_num/MBAR * SQRT(16.0_num * kb_0/(pi*MBAR)) * sigma_in)**(-1)
       ...
       DO iy = 0, ny 
          DO ix = 0, nx 
             ...
             DO izp=iz,iz+1
                DO iyp=iy,iy+1
                   DO ixp=ix,ix+1
                      CALL Get_Temp(rho(ixp,iyp,izp),energy(ixp,iyp,izp),eos_number,ixp,iyp,izp,T)
                      T_v=T_v+T
                   ENDDO
                ENDDO
             ENDDO
    ...
    DO iz = -1, nz+2
       DO iy = -1, ny+2 
          DO ix = -1, nx+2 
             rho0 = rho(ix,iy,iz)
             e0 = energy(ix,iy,iz)
             Ta = (gamma - 1.0_num) * (/  MAX((e0 - ionise_pot_local) / 2.0_num, none_zero), e0 / 2.0_num /)
             IF (Ta(1) > Ta(2)) THEN 
