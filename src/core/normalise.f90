...
MODULE normalise
  ...
  REAL(num) :: mf 
  !Average mass of a particle (assuming even mix of hydrogen ions and electrons)
  REAL(num) :: MBAR
...
CONTAINS
  ...
  SUBROUTINE Derived_Quantities
    ...
    !Average particle mass is mass of proton * mass of average particle in proton masses
    MBAR = mh * mf
    ...
    VISC0  = 1.0_num         !viscosity (Input as inverse Reynolds so L0*VEL0*RHO0 not needed)
    RES0   = 1.0_num         !resistivity (Input as inverse Lundquist so L0*VEL0*MU0 not needed)
    PRESSURE0 = B0**2/MU0  !Pressure
    TEMP0  = MBAR * PRESSURE0 /(KB * RHO0) !Temperature in K
    J0 = B0/(L0 * MU0)
    !Thermal conductivity
    KAPPA0 = ENERGY0**(3.0_num / 2.0_num) * RHO0 * L0 / (MBAR / KB * ENERGY0) ** (7.0_num / 2.0_num)
