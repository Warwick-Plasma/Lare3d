...
CONTAINS
  ...
  SUBROUTINE grid
    ...
    END DO
    zb_global = zb_global * length_z + z_start
    ...
    IF (zbc_max == BC_PERIODIC) THEN
      ...
      zb_global(-1) = zb_global(0) &
          - (zb_global(nz) - zb_global(nz-1))
      zb_global(-2) = zb_global(0) &
          - (zb_global(nz) - zb_global(nz-2))
    ELSE
  ...
  SUBROUTINE restart_data
    ...
    INTEGER, DIMENSION(3) :: dims, global_dims
    REAL(dbl) :: time_d
    ...
    DO ix = 1, nblocks
      ...
      ELSE IF (type == TYPE_MESH_VARIABLE) THEN
        ...
        IF (ANY(dims(1:3) /= global_dims(1:3))) THEN
