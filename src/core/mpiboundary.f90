...
CONTAINS
  ...
  SUBROUTINE bfield_MPI
    ...
    CALL MPI_SENDRECV(bx(nx-2:nx, 1:ny, :), 3*ny*(nz+4), mpireal, right, tag, &
        bx(-2:0, 1:ny, :), 3*ny*(nz+4), mpireal, left, tag, comm, &
        status, errcode)
    ...
    CALL MPI_SENDRECV(by(nx-1:nx, 1:ny, :), 2*ny*(nz+4), mpireal, right, tag, &
        by(-1:0, 1:ny, :), 2*ny*(nz+4), mpireal, left, tag, comm, &
        status, errcode)
    ...
    CALL MPI_SENDRECV(bz(nx-1:nx, 1:ny, :), 2*ny*(nz+5), mpireal, right, tag, &
        bz(-1:0, 1:ny, :), 2*ny*(nz+5), mpireal, left, tag, comm, &
        status, errcode)

    CALL MPI_SENDRECV(bx(:, ny-1:ny, :), 2*(nx+5)*(nz+4), mpireal, up, tag, &
        bx(:, -1:0, :), 2*(nx+5)*(nz+4), mpireal, down, tag, comm, &
        status, errcode)
  ...
  SUBROUTINE energy_MPI
    ...
    CALL MPI_SENDRECV(energy(1:nx, 1:ny, 1:2), 2*nx*ny, mpireal, front, tag, &
        ...
        status, errcode)
    CALL MPI_SENDRECV(energy(1:nx, 1:ny, nz-1:nz), 2*nx*ny, mpireal, back, &
        tag, energy(1:nx, 1:ny, -1:0), 2*nx*ny, mpireal, front, tag, comm, &
        status, errcode)

    CALL MPI_SENDRECV(energy(1:2, 1:ny, :), 2*ny*(nz+4), mpireal, left, tag, &
        energy(nx+1:nx+2, 1:ny, :), 2*ny*(nz+4), mpireal, right, tag, comm, &
        status, errcode)
    CALL MPI_SENDRECV(energy(nx-1:nx, 1:ny, :), 2*ny*(nz+4), mpireal, right, &
        tag, energy(-1:0, 1:ny, :), 2*ny*(nz+4), mpireal, left, tag, comm, &
        status, errcode)

    CALL MPI_SENDRECV(energy(:, ny-1:ny, :), 2*(nx+4)*(nz+4), mpireal, up, &
        tag, energy(:, -1:0, :), 2*(nx+4)*(nz+4), mpireal, down, tag, comm, &
        status, errcode)
  ...
  SUBROUTINE velocity_MPI
    ...
    CALL MPI_SENDRECV(vy(:, :, nz-2:nz), 3*xy_buf, mpireal, back, tag, &
        vy(:, :, -2:0), 3*xy_buf, mpireal, front, tag, comm, &
        status, errcode)
  ...
  SUBROUTINE remap_v_MPI
    ...
    CALL MPI_SENDRECV(vy1(:, :, nz-2:nz), 3*xy_buf, mpireal, back, tag, &
        vy1(:, :, -2:0), 3*xy_buf, mpireal, front, tag, comm, &
        status, errcode)
