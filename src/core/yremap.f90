...
CONTAINS
  ...
  SUBROUTINE remap_y
    ...
    DO iz = -1, nz + 1
      DO iy = -1, ny + 1
        DO ix = -1, nx + 1
          dyc1(ix,iy,iz) = 0.5_num * (dyb1(ix,iy,iz) + dyb1(ix,iy+1,iz))
        END DO
      END DO
    END DO
  ...
  SUBROUTINE dm_y_bcs
    ...
    CALL MPI_SENDRECV( &
        dm(0:nx+1,1   ,0:nz+1), (nx+2)*(nz+2), mpireal, proc_y_min, tag, &
        dm(0:nx+1,ny+1,0:nz+1), (nx+2)*(nz+2), mpireal, proc_y_max, tag, &
        comm, status, errcode)
