...
CONTAINS
    ...
    DO iz = 0, nz
      ...
      DO iy = 0, ny
        ...
        DO ix = 0, nx
          ...
          db    = (dyb1(ix, iy  , iz) + dyb1(ixp, iy  , iz)) * 0.5_num
          ...
          w6 = SIGN(1.0_num, v_advect) * w8 &
              * MIN(ABS(w9) * (dyb(iy) * vad_p + dyb(iyp) * vad_m), &
    ...
    DO iz = 0, nz
      ...
      DO iy = 0, ny
        ...
        DO ix = 0, nx
          ...
          db    = (dyb1(ix, iy  , iz) + dyb1(ix, iy  , izp)) * 0.5_num
    ...
    DO iz = 0, nz
      ...
      DO iy = 0, ny
        ...
        DO ix = 0, nx
          ...
          flux(ix, iy, iz) = dm(ix, iy, iz) * (w7 + w9 * (1.0_num - w6))
        END DO
      END DO
    END DO
    ...
    DO iz = 0, nz
      DO iy = -1, ny
        ...
        DO ix = 0, nx
          ...
          flux(ix, iy, iz) = w7 + w9 * (1.0_num - w6)
        END DO
      END DO
    END DO
  ...
  SUBROUTINE dm_y_bcs
    ...
    CALL MPI_SENDRECV(dm(0:nx+1, 1, 0:nz+1), (nx+2)*(nz+2), mpireal, &
        proc_y_min, tag, dm(0:nx+1, ny+1, 0:nz+1), (nx+2)*(nz+2), mpireal, &
    ...
    CALL MPI_SENDRECV(dm(0:nx+1, ny-1, 0:nz+1), (nx+2)*(nz+2), mpireal, &
        proc_y_max, tag, dm(0:nx+1, -1, 0:nz+1), (nx+2)*(nz+2), mpireal, &
