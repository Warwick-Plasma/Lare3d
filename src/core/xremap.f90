...
CONTAINS
    ...
    DO iz = 0, nz
      ...
      DO iy = 0, ny
        ...
        DO ix = 0, nx
          ...
          db    = (dxb1(ix  , iy, iz) + dxb1(ix  , iyp, iz)) * 0.5_num
          ...
          w6 = SIGN(1.0_num, v_advect) * w8 &
              * MIN(ABS(w9) * (dxb(ix) * vad_p + dxb(ixp) * vad_m), &
    ...
    DO iz = 0, nz
      ...
      DO iy = 0, ny
        ...
        DO ix = 0, nx
          ...
          db    = (dxb1(ix  , iy, iz) + dxb1(ix  , iy, izp)) * 0.5_num
  ...
  SUBROUTINE dm_x_bcs
    ...
    CALL MPI_SENDRECV(dm(1, 0:ny+1, 0:nz+1), (ny+2)*(nz+2), mpireal, &
        proc_x_min, tag, dm(nx+1, 0:ny+1, 0:nz+1), (ny+2)*(nz+2), mpireal, &
    ...
    CALL MPI_SENDRECV(dm(nx-1, 0:ny+1, 0:nz+1), (ny+2)*(nz+2), mpireal, &
        proc_x_max, tag, dm(-1, 0:ny+1, 0:nz+1), (ny+2)*(nz+2), mpireal, &
