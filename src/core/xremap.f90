...
MODULE xremap
  ...
  REAL(num), DIMENSION(:,:,:), ALLOCATABLE :: rho1, dm, cv2, flux, dxb1, rho_v, rho_v1
  ...
  SUBROUTINE remap_x !remap onto original Eulerian grid
    ...
    DO iz = -1, nz+2
       DO iy = -1, ny+2
          DO ix = -1, nx+2
             ...
             cv1(ix,iy,iz) = cv(ix,iy,iz) * (1.0_num + dv)  !control volume before remap
             dv = (REAL(ypass,num) * (vyb - vybm)/dyb(iy) &
                  + REAL(zpass,num) * (vzb - vzbm)/dzb(iz)) * dt
             cv2(ix,iy,iz) = cv(ix,iy,iz) * (1.0_num + dv)  !control volume after remap
             dxb1(ix,iy,iz) = dxb(ix) + (vxb - vxbm) * dt !dxb before remap
          END DO
       END DO
    END DO
    ...
    DO iz = 1, nz
       DO iy = 1, ny
          DO ix = 1, nx
             ixm = ix - 1
             rho(ix,iy,iz) = (rho1(ix,iy,iz)*cv1(ix,iy,iz) + dm(ixm,iy,iz)-dm(ix,iy,iz)) &
                  /cv2(ix,iy,iz)
          END DO
       END DO
    END DO
    ...
    DO iz = 1, nz
       DO iy = 1, ny
          DO ix = 1, nx
             ixm = ix - 1
             energy(ix,iy,iz) = (energy(ix,iy,iz)*cv1(ix,iy,iz)*rho1(ix,iy,iz) &
                  +flux(ixm,iy,iz)-flux(ix,iy,iz))/(cv2(ix,iy,iz)*rho(ix,iy,iz))
          END DO
       END DO
    END DO
    ...
    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = -1, nx+1    ! vertex density before remap
             ixp = ix + 1
             iyp = iy + 1
             izp = iz + 1
             ...
             rho_v(ix,iy,iz) = rho_v(ix,iy,iz)/(cv1(ix,iy,iz) + cv1(ixp,iy,iz)  &
                  + cv1(ix,iyp,iz) + cv1(ixp,iyp,iz) + cv1(ix,iy,izp) + cv1(ixp,iy,izp)  &
    ...
    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = 0, nx
             ...
             iyp = iy + 1
             izp = iz + 1
             flux(ix,iy,iz) = cv1(ix,iy,iz) + cv1(ixp,iy,iz) + cv1(ix,iyp,iz) &
                  + cv1(ixp,iyp,iz) + cv1(ix,iy,izp) + cv1(ixp,iy,izp) &
                  + cv1(ix,iyp,izp) + cv1(ixp,iyp,izp)
          END DO
       END DO
    END DO
    cv1(0:nx,0:ny,0:nz) = flux(0:nx,0:ny,0:nz) / 8.0_num  ! cv1 = vertex CV before remap
    ...
    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = 0, nx
             ...
             iyp = iy + 1
             izp = iz + 1
             flux(ix,iy,iz) = cv2(ix,iy,iz) + cv2(ixp,iy,iz) + cv2(ix,iyp,iz) &
                  + cv2(ixp,iyp,iz) + cv2(ix,iy,izp) + cv2(ixp,iy,izp) & 
                  + cv2(ix,iyp,izp) + cv2(ixp,iyp,izp)
          END DO
       END DO
    END DO
    cv2(0:nx,0:ny,0:nz) = flux(0:nx,0:ny,0:nz) / 8.0_num  ! cv2 = vertex CV after remap
    ...
    DO iz = 0, nz
       ...
       END DO
    END DO
    vx1(-2:nx+1,0:ny,0:nz) = flux(-2:nx+1,0:ny,0:nz) ! vertex boundary velocity used in remap

    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = -1, nx+1    !dxb1 = width of vertex CV before remap
             ixm = ix - 1
    ...
    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = -1, nx
             ...
             iyp = iy + 1
             izp = iz + 1
             flux(ix,iy,iz) = dm(ix,iy,iz) + dm(ixp,iy,iz) + dm(ix,iyp,iz) &
                  + dm(ixp,iyp,iz) + dm(ix,iy,izp) + dm(ixp,iy,izp) &
                  + dm(ix,iyp,izp) + dm(ixp,iyp,izp)
          END DO
       END DO
    END DO
    dm(-1:nx,0:ny,0:nz) = flux(-1:nx,0:ny,0:nz) / 8.0_num  ! mass flux out of vertex CV

    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = 0, nx        ! vertex density after remap
             ixm = ix - 1
             rho_v1(ix,iy,iz) = (rho_v(ix,iy,iz)*cv1(ix,iy,iz)+dm(ixm,iy,iz) &
                  -dm(ix,iy,iz))/cv2(ix,iy,iz)
          END DO
       END DO
    END DO
    ...
    DO iz = 0, nz
       DO iy = 0, ny 
          DO ix = 0, nx
             ixm = ix - 1
             vy(ix,iy,iz) = (rho_v(ix,iy,iz)*vy(ix,iy,iz)*cv1(ix,iy,iz)  &
                  +flux(ixm,iy,iz)-flux(ix,iy,iz))/(cv2(ix,iy,iz)*rho_v1(ix,iy,iz))
          END DO
       END DO
    END DO
    ...
    DO iz = 0, nz
       DO iy = 0, ny 
          DO ix = 0, nx
             ixm = ix - 1
             vz(ix,iy,iz) = (rho_v(ix,iy,iz)*vz(ix,iy,iz)*cv1(ix,iy,iz)  &
                  +flux(ixm,iy,iz)-flux(ix,iy,iz))/(cv2(ix,iy,iz)*rho_v1(ix,iy,iz))
          END DO
       END DO
    END DO
    ...
    DO iz = 0, nz
       DO iy = 0, ny
          DO ix = 0, nx
             ixm = ix - 1
             vx(ix,iy,iz) = (rho_v(ix,iy,iz)*vx(ix,iy,iz)*cv1(ix,iy,iz)  &
                  +flux(ixm,iy,iz)-flux(ix,iy,iz))/(cv2(ix,iy,iz)*rho_v1(ix,iy,iz))
          END DO
       END DO
    END DO
    ...
    DO iz = 0, nz 
       DO iy = 0, ny 
          ...
          DO ix = 0, nx
             ...
             flux(ix,iy,iz) = (MAX(0.0_num, v_advect)*w4 + MIN(0.0_num, v_advect)*w5) * dt

             w1 = by(ixp,iy,iz) / dbxp - by(ix,iy,iz) / db
             w2 = by(ix,iy,iz) / db - by(ixm,iy,iz) / dbxm 
             w3 = by(ixp2,iy,iz) / dbxp2 - by(ixp,iy,iz) / dbxp

             !vad_p and vad_m are logical switches which determine v_advect>=0 and v_advect<0 respectivly
             !It's written this way to allow vector optimization

             vad_p=-MIN(SIGN(1.0_num,-v_advect),0.0_num)
             vad_m=MAX(SIGN(1.0_num,-v_advect),0.0_num)

             w5 = ABS(v_advect) * dt / (db * vad_p + dbxp *vad_m)
             w4 = (2.0_num-w5)*ABS(w1)/dxc(ix)+(1.0_num+w5)*ABS(w2*vad_p + w3*vad_m)&
                  /(dxc(ixm)*vad_p+dxc(ixp)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,(w2*vad_p+w3*vad_m)))
             w6 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*((dxb(ix)*vad_p+dxb(ix+1)*vad_m))&
                  , ABS(w1), ABS(w2*vad_p+w3*vad_m))
             flux(ix,iy,iz) = flux(ix,iy,iz) + v_advect * dt * w6 * (1.0_num - w5)
    ...
    DO iz = 0, nz 
       DO iy = 0, ny 
          ...
          DO ix = 0, nx
             ...
             !vad_p and vad_m are logical switches which determine v_advect>=0 and v_advect<0 respectivly
             !It's written this way to allow vector optimization. See example in **SECTION 2**
             vad_p=-MIN(SIGN(1.0_num,-v_advect),0.0_num)
             vad_m=MAX(SIGN(1.0_num,-v_advect),0.0_num)

!This code is the vectorizable replacement for the code in **SECTION 2**
             w5 = ABS(v_advect) * dt / (db*vad_p+dbxp*vad_m)
             w4 = (2.0_num-w5)*ABS(w1)/dxc(ix)+(1.0_num+w5)*ABS(w2*vad_p+w3*vad_m)&
                  /(dxc(ixm)*vad_p+dxc(ixp)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
             w6 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dxb(ix)*vad_p+dxb(ixp)*vad_m)&
                  , ABS(w1), ABS(w2*vad_p+w3*vad_m))
             flux(ix,iy,iz) = flux(ix,iy,iz) + v_advect * dt * w6 * (1.0_num - w5)
...
!!$                w4 = (2.0_num-w5)*ABS(w1)/dxc(ix)+(1.0_num+w5)*ABS(w2)/dxc(ixm)
!!$                w4 = w4 / 6.0_num
!!$                w4 = ABS(w1)/dxc(ix)
!!$                w4 = w4 / 2.0_num
...
!!$                w4 = (2.0_num-w5)*ABS(w1)/dxc(ix)+(1.0_num+w5)*ABS(w3)/dxc(ixp)
!!$                w4 = w4 / 6.0_num
...
!!$             !**END SECTION 2**
    ...
    DO iz = 0, nz + 1
       DO iy = 0, ny+1
          ...
          DO ix = 0, nx
             ...
             v_advect = (vx1(ix,iy,iz)+vx1(ix,iym,iz)+vx1(ix,iy,izm)+vx1(ix,iym,izm))/4.0_num
             ...
             w5 = ABS(v_advect) * dt / (dxb1(ix,iy,iz)*vad_p+dxb1(ixp,iy,iz)*vad_m)
             w4 = (2.0_num-w5)*ABS(w1)/dxc(ix)+(1.0_num+w5)*ABS(w2*vad_p+w3*vad_m)&
                  /(dxc(ixm)*vad_p+dxc(ixp)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,(w2*vad_p + w3*vad_m)))
    ...
    DO iz = 0, nz 
       DO iy = 0, ny 
          ...
          DO ix = 0, nx
             ...
             w6 = ABS(dm(ix,iy,iz)) &
                  / ((rho1(ix,iy,iz)*vad_p + rho1(ixp,iy,iz)*vad_m)*(dxb1(ix,iy,iz)*vad_p &
                  + dxb1(ixp,iy,iz)*vad_m)*dyb(iy)*dzb(iz))
             w4 = (2.0_num-w5)*ABS(w1)/dxc(ix)+(1.0_num+w5)*ABS(w2*vad_p+w3*vad_m)&
                  /(dxc(ixm)*vad_p+dxc(ixp)*vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
             w5 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dxb(ix)*vad_p&
                  +dxb(ixp)*vad_m), ABS(w1), ABS(w2*vad_p+w3*vad_m))
    ...
    DO iz = 0, nz 
       DO iy = 0, ny 
          ...
          DO ix = -1, nx
             ...
             w5 = ABS(v_advect) * dt / (dxb1(ix,iy,iz) * vad_p &
                  + dxb1(ixp,iy,iz) *vad_m)
             w7 = vy(ix,iy,iz) * vad_p + vy(ixp,iy,iz) * vad_m
             w6 = ABS(dm(ix,iy,iz)) &
                  / ((rho_v(ix,iy,iz) * dxb1(ix,iy,iz)*vad_p &
                  + rho_v(ixp,iy,iz) * dxb1(ixp,iy,iz) * vad_m)*dyc(iy)*dzc(iz))
             w4 = (2.0_num-w5)*ABS(w1)/dxb(ixp)+(1.0_num+w5)&
                  *ABS(w2*vad_p + w3*vad_m)/(dxb(ix)*vad_p + dxb(ixp2) *vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
             w5 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dxc(ix)*vad_p+dxc(ixp)*vad_m)&
                  , ABS(w1), ABS(w2*vad_p+w3*vad_m))
       ...
       DO iz = 0, nz 
          DO iy = 0, ny 
             DO ix = 0, nx-1
                ...
                dk = (vy(ixp,iy,iz) - vy(ix,iy,iz))  &
                     * (flux(ix,iy,iz) - 0.5_num*(vy(ixp,iy,iz)+vy(ix,iy,iz)))  &
       ...
       DO iy = 0, ny 
          ...
          DO ix = -1, nx
             ...
             w4 = (2.0_num-w5)*ABS(w1)/dxb(ixp)&
                  + (1.0_num+w5)*ABS(w2*vad_p + w3 * vad_m)/(dxb(ix) * vad_p + dxb(ixp2) *vad_m)
             w4 = w4 / 6.0_num
             w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p + w3*vad_m))
             w5 = SIGN(1.0_num,v_advect) * w8 * MIN(ABS(w4)*(dxc(ix)*vad_p + dxc(ixp)*vad_m)&
                  , ABS(w1), ABS(w2*vad_p + w3*vad_m))
       ...
       DO iz = 0, nz 
          DO iy = 0, ny 
             DO ix = 0, nx-1
                ...
                dk = (vz(ixp,iy,iz) - vz(ix,iy,iz))   &
                     * (flux(ix,iy,iz) - 0.5_num*(vz(ixp,iy,iz)+vz(ix,iy,iz)))  &
       ...
       DO iy = 0, ny 
          ...
          DO ix = -1, nx
             ...
             vad_m=MAX(SIGN(1.0_num,-v_advect),0.0_num)
                ...
                w4 = w4 / 6.0_num
                w8 = 0.5_num * (SIGN(1.0_num,w1) + SIGN(1.0_num,w2*vad_p+w3*vad_m))
                w5 = SIGN(1.0_num,v_advect)  * w8 * MIN(ABS(w4)*dxc(ix), ABS(w1)&
                     , ABS(w2*vad_p+w3*vad_m))
       ...
       DO iz = 0, nz 
          DO iy = 0, ny 
             DO ix = 0, nx-1
                ...
                dk = (vx(ixp,iy,iz) - vx(ix,iy,iz))   &
                     * (flux(ix,iy,iz) - 0.5_num*(vx(ixp,iy,iz)+vx(ix,iy,iz)))  &
  ...
  SUBROUTINE dm_x_bcs
    ...
    CALL MPI_SENDRECV(dm(1,0:ny+1,0:nz+1), (ny+2)*(nz+2), mpireal, left, tag,  &
         dm(nx+1,0:ny+1,0:nz+1), (ny+2)*(nz+2), mpireal, right, tag, comm, status, errcode)
    IF (right == MPI_PROC_NULL) dm(nx+1,0:ny+1,0:nz+1) =  dm(nx,0:ny+1,0:nz+1)

    CALL MPI_SENDRECV(dm(nx-1,0:ny+1,0:nz+1), (ny+2)*(nz+2), mpireal, right, tag,  &
         dm(-1,0:ny+1,0:nz+1), (ny+2)*(nz+2), mpireal, left, tag, comm, status, errcode)
    IF (left == MPI_PROC_NULL) dm(-1,0:ny+1,0:nz+1) =  dm(0,0:ny+1,0:nz+1)
