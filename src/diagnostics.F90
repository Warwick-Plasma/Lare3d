...
CONTAINS
  ...
  SUBROUTINE output_routines(i)   ! i=step index
    ...
    IF (print_arrays) THEN   ! output a snapshot of arrays
       ...
       IF (DumpMask(1))  THEN
          ...
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("Rho",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(2))  THEN
          Data=energy(0:nx,0:ny,0:nz)*ENERGY0
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("Energy",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(3))  THEN
          Data=vx(0:nx,0:ny,0:nz)*VEL0
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("Vx",&
               "Velocity",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(4))  THEN
          Data=vy(0:nx,0:ny,0:nz)*VEL0
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("Vy",&
               "Velocity",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(5))  THEN
          Data=vz(0:nx,0:ny,0:nz)*VEL0
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("Vz",&
               "Velocity",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(6))  THEN
          ...
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("Bx",&
               "Magnetic_Field",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       IF (DumpMask(7))  THEN
       ...
       IF (DumpMask(9)) THEN
          DO iz=0,nz
             DO iy=0,ny
                DO ix=0,nx
                   CALL Get_Temp(rho(ix,iy,iz),energy(ix,iy,iz),eos_number,ix,iy,iz,Data(ix,iy,iz))
                ENDDO
             ENDDO
          ENDDO
          Data=Data * TEMP0
       ...
       IF (DumpMask(10))  THEN
          DO iz=0,nz
             DO iy=0,ny
                DO ix=0,nx
                   CALL Get_Pressure(rho(ix,iy,iz),energy(ix,iy,iz),eos_number,ix,iy,iz,Data(ix,iy,iz))
                ENDDO
             ENDDO
          ENDDO
          Data=Data * PRESSURE0
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("Pressure",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(11)) THEN
          DO iz=0,nz
             DO iy=0,ny
                DO ix=0,nx
                   CALL Get_Cs(rho(ix,iy,iz),energy(ix,iy,iz),eos_number,ix,iy,iz,Data(ix,iy,iz))
                ENDDO
             ENDDO
          ENDDO
          Data=Data * VEL0
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("cs",&
               "Fluid",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(12)) THEN
          ...
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("j_par",&
               "PIP",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF

       IF (DumpMask(13)) THEN
          Data= perp_current(0:nx,0:ny,0:nz) * J0
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("j_perp",&
               "PIP",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(15)) THEN
          ...
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("eta_perp",&
               "PIP",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       IF (DumpMask(16)) THEN
          ...
          CALL cfd_Write_3D_Cartesian_Variable_Parallel("eta",&
               "PIP",Dims,Stagger,"Grid"&
               ,"Grid",data,subtype)
       ENDIF
       ...
       Data = rank
       CALL cfd_Write_3D_Cartesian_Variable_Parallel("rank",&
            "parallel",Dims,Stagger,"Grid"&
            ,"Grid",data,subtype)
#endif
    ...
    DO iz = -1, nz+2
       DO iy = -1, ny+2
          DO ix = -1, nx+2
            ...
            dxlocal = 1.0_num / (1.0_num/dxb(ix)**2 + 1.0_num/dyb(iy)**2 + 1.0_num/dzb(iz)**2)
            IF (cowling_resistivity) THEN
               dt3 = 0.2_num * dxlocal / MAX(MAX(eta(ix,iy,iz),eta_perp(ix,iy,iz)), none_zero)
            ELSE
  ...
  SUBROUTINE output_log  !writes basic data to 'lare2d.dat'
    ...
    WRITE(20,*) 'Thermal conductivity normalisation = ', KAPPA0,' kgm^(-1)s^(-1) '
