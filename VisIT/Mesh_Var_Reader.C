...
//--------------------------------------------------------------------------------------------
 Mesh_Var_Reader::Mesh_Var_Reader(BlockHandler *Handler,ifstream *file,Block* Owner,int MaxStringLen,bool CacheOnly)
...
vtkDataSet* Mesh_Var_Reader::GetMesh(int domain)
{
	...
	for (int i = 0 ; i < this->Dims[0]; i++)
	  {
	    if (this->SizeOfFloat==4)Data->SetTuple3(i, ((float*)v1)[i], ((float*)v2)[i], 0.0);
	    else Data->SetTuple3(i, ((double*)v1)[i], ((double*)v2)[i], 0.0);
	  }
        pts->SetData(Data);
        Data->Delete();
	free(v1);
...
InternalMetaData* Mesh_Var_Reader::GetInternalMetaData()
{
    ...
    if (this->VarType == VAR_PARTICLE) MD->n_Elements=this->nPart;
