...
//--------------------------------------------------------------------------------------------
 Stitched_Vector_Reader::Stitched_Vector_Reader(BlockHandler *Handler,ifstream *file,Block* Owner,int MaxStringLen,bool CacheOnly)
...
void Stitched_Vector_Reader::PopulateDatabaseMetaData(avtDatabaseMetaData *md)
{
    ...
    avtVectorMetaData *vmd = new avtVectorMetaData(Composite,MeshComposite,cent,this->Dimensions);
    md->Add(vmd);
...
//Read the subblocks
	...
	debug1 << SubBlocks[dimswing]->Offset + SubBlocks[dimswing]->Block_MD_Length << endl;
	...
	while (nel_left >0) {
	    ...
	    {
		memcpy(pointdata,((char*)v)+i*this->SizeOfFloat,this->SizeOfFloat);
		//Move pointer
